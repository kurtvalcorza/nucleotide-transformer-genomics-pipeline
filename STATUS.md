...
Because both gates are open, this repository ships the model card, the standalone tutorial notebook, the pinned snapshot manifest and the supporting documentation — and deliberately **no** DIMER pipeline module, adapter or release-asset validator. The exact committed notebook blob has recorded local CPU and clean Kaggle Tesla T4 executions (`docs/release-verification.md`), and the static checks (`tools/make_notebook.py --check`, `tools/check_assets.py`) pass. These facts establish technical execution, not DIMER readiness.
