"""Static release-asset validation for the Nucleotide Transformer v2 50M genomics DIMER pipeline.
...
EXPECTED_MODEL_ID = "InstaDeepAI/nucleotide-transformer-v2-50m-multi-species"
...
# Additional 40-hex revisions a document may legitimately cite: the genomic_benchmarks commit the interval
# lists are pinned to (samples.py CORPUS_REVISION).
KNOWN_SHAS: frozenset[str] = frozenset(("605d8539830e16c85abe7826990958303ffc5e1c",))
...
EXPECTED_OUTPUTS = (
    "outputs/nucleotide_transformer_train.csv",
    "outputs/nucleotide_transformer_input_manifest.json",
    "outputs/nucleotide_transformer_embeddings.csv",
    "outputs/nucleotide_transformer_evaluation_report.json",
    "outputs/nucleotide_transformer_adapter",
    "outputs/nucleotide_transformer_result.json",
)
...
CODE_MARKERS = (
    ...
    "'corpus': {'name': CORPUS_NAME, 'repo': CORPUS_REPO, 'revision': CORPUS_REVISION, 'dataset': CORPUS_DATASET, 'license': CORPUS_LICENSE",
    "transformers.__version__",
    "'device': pipe.device",
)
# Profile-specific learner-facing statements.
MARKDOWN_MARKERS = (
    "**Capability:** DNA sequence representation (one 512-d mean-pooled vector per sequence) and bounded supervised fine-tuning of a promoter classifier",
    "**CC BY-NC-SA 4.0**",
    "**commercial use is not permitted under this licence**",
    "**This checkpoint ships its own model code**",
    "`trust_remote_code=True`",
    "**adaptation with labelled sequences**",
    "**0.715**",
    "**0.693**",
    "**mean-pooled head**",
    "**validation MCC**",
    ...
    "**Snapshot note:**",
    "**the licence the adapter inherits (`cc-by-nc-sa-4.0`)**",
    ...
    "**Leakage:**",
    "nothing it produces may be used commercially",
)
...
EXTERNAL_WEIGHT_BYTES: dict[int, str] = {
    34_645_456: "nucleotide-transformer adapter.safetensors, default recipe (32 tensors)",
    223_682_957: "InstaDeepAI/nucleotide-transformer-v2-50m-multi-species 81b29e5 pytorch_model.bin",
}
EXTERNAL_WEIGHT_DIGESTS: dict[str, str] = {
    "d913c4bd4cf74199456d24ced1f3ddc8f39e9241ab2eca132ffb9885efc62cfb": "SAMPLE_DIGEST: dataset_digest over the 2,200 inline promoter windows",
}
...
def _enabling_calls(text: str) -> int:
    """Call sites passing trust_remote_code=True, counted on the AST so docstrings and prose do not count."""
    count = 0
    for node in ast.walk(ast.parse(text)):
        if isinstance(node, ast.Call):
            for keyword in node.keywords:
                if keyword.arg == "trust_remote_code" and isinstance(keyword.value, ast.Constant) and keyword.value.value is True:
                    count += 1
    return count


def _validate_remote_code_perimeter(path: Path, stripped: dict[int, str], embedded: list[int]) -> None:
    """The one deliberate exception to the fleet's remote-code ban (accepted for this row on 2026-09-20):
    `trust_remote_code=True` may be passed only inside the carried pipeline module, exactly once, inside
    `from_pretrained`, after `verify_snapshot(root)` has run in that same method; nothing outside the carried
    modules may enable it, and the tokenizer load must stay native."""
    outside_calls = sum(_enabling_calls(text) for index, text in stripped.items() if index not in embedded)
    _check(outside_calls == 0, f"{path.name}: trust_remote_code=True outside the carried module cells")
    inside = "\n".join(text for index, text in stripped.items() if index in embedded)
    hits = sum(_enabling_calls(text) for index, text in stripped.items() if index in embedded)
    _check(hits == 1, f"{path.name}: expected exactly one trust_remote_code=True call in the carried module, found {hits}")
    method = re.search(r"    def from_pretrained\(.*?\n    def ", inside, re.S)  # comments are already stripped
    _check(method is not None, f"{path.name}: from_pretrained not found in the carried module")
    body = method.group(0)
    verify_at, load_at = body.find("verify_snapshot(root)"), body.find("trust_remote_code=True")
    _check(0 <= verify_at < load_at, f"{path.name}: verify_snapshot must run before the remote-code load inside from_pretrained")
    _check("AutoTokenizer.from_pretrained(str(root), local_files_only=True, trust_remote_code=False)" in body, f"{path.name}: the tokenizer must load natively (trust_remote_code=False)")
    _check("local_files_only=True, trust_remote_code=True" in body, f"{path.name}: the remote-code load must be local_files_only")
    _check('REMOTE_CODE_FILES = ("modeling_esm.py", "esm_config.py")' in inside, f"{path.name}: the carried module must name the remote-code files")
